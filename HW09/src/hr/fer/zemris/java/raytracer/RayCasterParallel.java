...
/**
 * The class is used for drawing certain 3D scene with objects in it. In order to achieve that, 
 * we are using ray-tracing technique. Also, all calculation tasks are divided into smaller ones
 * to achieve parallelization of the tasks. In this example, we are using implementation of the 
 * {@link ExecutorService} - {@link ForkJoinPool} and {@link RecursiveAction} for that type of 
 * parallelization.
 * 
...
public class RayCasterParallel {
	...
	/** The treshold for difference between y-coordinates. */
	 ...
	 * to see the scene and the objects that define the scene.
	 *
	...
	private static IRayTracerProducer getIRayTracerProducer() {
		return new IRayTracerProducer() {
			...
			public void produce(Point3D eye, Point3D view, Point3D viewUp, double horizontal, double vertical,
				...
				/**
				 * Local class representing implementation of the {@link ForkJoinTask}.
				 * Every instance of this class calculates pixel's colors intensity between 
				 * minimal and maximal y-coordinate. If difference between {@code ymin} and 
				 * {@code ymax} is bigger than predefined treshold, task is divided into two 
				 * smaller tasks. Both of the newly instantiated tasks are invoked by an instance
				 * of the {@link ForkJoinPool}. This task dividing continues until the task is 
				 * small enough to be computed directly.
				 * 
				 * @author Vjeco
				 */
				class PixelPaint extends RecursiveAction {
					/**
					 * Serial version ID.
					 */
					private static final long serialVersionUID = 1L;
					
					/** Minimal value of the y-coordinate. */
					private int ymin;
					/** Maximal value of the y-coordinate. */
					private int ymax;

					/**
					 * Instantiates and new pixel paint task with given parameters.
					 * 
					 * @param ymin minimal value of the y-coordinate
					 * @param ymax maximal value of the y-coordinate
					 */
					public PixelPaint(int ymin, int ymax) {
						this.ymin = ymin;
						this.ymax = ymax;
					}
					...
					protected void compute() {
						...
						int mid = (ymin + ymax) / 2;
						PixelPaint pp1 = new PixelPaint(ymin, mid);
						PixelPaint pp2 = new PixelPaint(mid, ymax);
						invokeAll(pp1, pp2);
					}

					/**
					 * Helper method that computes the given task directly, without
					 * dividing it to two smaller tasks.
					 */
				...
				PixelPaint pp = new PixelPaint(0, height);
				pool.invoke(pp);
			...
			private Point3D calculateScreenCorner(Point3D view, Point3D xAxis, Point3D yAxis, double horizontal, double vertical) {
				return view.sub(xAxis.scalarMultiply(horizontal / 2)).modifyAdd(yAxis.scalarMultiply(vertical / 2));
			}
	...
	/**
	 * Method for calculating colors of the every pixel in the frame. If there is no ray-object 
	 * intersection, pixel's every color component intensity is set to 0. However, if intersection 
	 * exists, the closest intersection of the given ray and any object in the scene is taken.
	 *
	 ...
	 */
	private static void determineColorFor(Scene scene, List<GraphicalObject> objects,
										  RayIntersection intersection, short[] rgb, Point3D eye) {
		setRgb((short) 15, rgb);
		...
		for (LightSource ls : lightSources) {
			...
			if (closestIntersection != null && ls.getPoint().sub(closestIntersection.getPoint()).norm()
					+ EPS < ls.getPoint().sub(intersection.getPoint()).norm()) {
				continue;
			} else {
	...
	private static void addDiffuseAndReflectiveComp(short[] rgb, LightSource ls, RayIntersection intersection, Point3D eye) {
		...
		double powN = pow(cosinusReflected,intersection.getKrn());
		if (cosinusReflected > 0) {
			rgb[0] += (short) (ls.getR() * intersection.getKrr() * powN);
			rgb[1] += (short) (ls.getG() * intersection.getKrg() * powN);
			rgb[2] += (short) (ls.getB() * intersection.getKrb() * powN);
		}
	}
