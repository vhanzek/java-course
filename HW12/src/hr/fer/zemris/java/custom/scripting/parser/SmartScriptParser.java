...
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.DOUBLE;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.FUNCTION;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.INTEGER;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.INVALID;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.OPERATOR;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.STRING;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.VALID;
import static hr.fer.zemris.java.custom.scripting.parser.ParserConstants.VARIABLE;
...
/**
 * The Class SmartScriptParser used for making structured document tree from elements 
 * given to parser by lexical analyser {@link Lexer}.
 * 
 * @author Vjeco
 */
public class SmartScriptParser {
	...
	/** The document stack. */
	private Stack<Node> documentStack;
	...
	/**
	 * <p>Method for parsing document text.</p>
	 ...
	 * If token type is TAG, then method determines tag type, parses it, adds the token as a child to document 
	 * node and, additionally, if tag type is FOR, it becomes new top level node for other nodes as long as the 
	 * end tag occurs.</p>
	 *
	...
	private void parse(String docBody){
		...
		while(token.getType() != TokenType.EOF){			
			if(token.getType() == TokenType.TEXT){
				...
				case FOR : 	List<Element> elements = parseTag(tagElements);
	...
	/**
	 * Method for parsing tag elements. Elements can be put without spaces and method can 
	 * still recognize element types.
	 * 
	 ...
	 */
	private List<Element> parseTag(String tagElements) {
			...
			Matcher matcher = Pattern.compile("(" + DOUBLE + ")|(" + INTEGER + ")|" + STRING + "|@(" + VALID +"+)|"
											+ "([a-zA-Z|\\_]" + VALID + "*)|(" + OPERATOR + ")|(" + INVALID + ")").matcher(tagElements);
	...
	/**
	 * Checks if number of elements in for loop node is legal and is certain 
	 * element instance of desired class.
	 *
	 * @throws SmartScriptParserException if number of elements is illegal or 
	 * 									  if certain element is not instance of desired class
	 * @param elements the elements for checking
	 * @return the new for loop node filled with elements
	 */
	private ForLoopNode checkElements(List<Element> elements) {			
	...
	/**
	 * Checks if element is legal. Element is legal if it's a 
	 * string, double or integer constant.
	 *
	...
	/**
	 * Checks if there is illegal escape sequence in given string. 
	 * Escape sequence is legal if it's character \ or ".
	 *
	...
	/**
	 * Checks if given string is valid variable name. Valid name starts with the 
	 * letter followed by letters, numbers or _.
	 *  
	...
	/**
	 * Checks if given string is valid function name. Valid name starts with 
	 * the letter followed by letters, numbers or _.
	 *  
	...
	private Object[] determineTagType(String tag) {
		...
		} else if (tempTag.startsWith("for ")){
			array[0] = tag.substring(4);
			array[1] = TagType.FOR;
		} else if (tempTag.startsWith("end")){
			array[0] = null;
			array[1] = TagType.END;
		} else if (tempTag.startsWith("=")){
	 ...
	 */
	public Stack<Node> getDocumentStack() {
